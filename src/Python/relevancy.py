...
def loadModel():
    global model
    print("RELEVANCY :: Chargement du modèle")
    filename = 'disasterRelevantModelMultiNB_alpha06.pkl'
    model = pickle.load(open(filename, 'rb'))
    print("RELEVANCY :: Modèle " + filename + " chargé")

#Renvois un booléen vrai si le texte parle d'une catastrophe, faux sinon
def predict(text):
    return (model.predict([text])=="Relevant")[0]
